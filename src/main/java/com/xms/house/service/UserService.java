...
import com.xms.house.common.UserException;
import com.xms.house.common.UserException.Type;
import com.xms.house.mapper.UserMapper;
import com.xms.house.model.User;
import com.xms.house.utils.BeanHelper;
import com.xms.house.utils.HashUtils;
import com.xms.house.utils.JwtHelper;
...
public class UserService {
  ...
  public boolean addAccount(User user, String enableUrl) {
    ...
    BeanHelper.onInsert(user);//设置了用户的创建时间以及更新时间
    userMapper.insert(user);//插入数据库
    registerNotify(user.getEmail(),enableUrl);//通知邮件
    return true;
  }
  ...
  private void registerNotify(String email, String enableUrl) {
    ...
    redisTemplate.opsForValue().set(randomKey, email);//存储到redis
    redisTemplate.expire(randomKey, 1,TimeUnit.HOURS);//设置redis的过期时间为1小时
    String content = enableUrl +"?key="+  randomKey;//发送内容
  ...
  private String renewToken(String token, String email) {
    ...
    redisTemplate.expire(email, 30, TimeUnit.MINUTES);//设置30分钟过期
    return token; 
  }
  ...
  public User getLoginedUserByToken(String token) {
    ...
    //如果redis的过期时间大于0 时间有效 重新重置redis中的token有效时间
    if (expired > 0L) {
